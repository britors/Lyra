...
KIWI image description for the Lyra OS "Odisseia" Beta 2 x86_64 live/installer
...
user invokes and confirms it; it has no timer, service, network request, or
upload path. It deliberately refuses root and existing output files. Raw
collection is held in a private temporary directory, passed through
`/usr/libexec/lyra-report-redact`, and deleted before the redacted report is
shown for review. The report covers live/installed context, release, hardware,
boot, network state without profiles, packages/repositories, Btrfs/Snapper,
GRUB and installer logs. Secret fixtures exercise the redactor in CI. See
`docs/diagnostics.md` for the review and manual attachment flow.
...
- **`/etc/os-release`** is generated by `config.sh` from `release.toml` to
  report `NAME="Lyra OS"`, the current `PRETTY_NAME` and `VERSION_ID`, and
...
O helper `test/build-and-run-vm.sh` valida o nome e o `VERSION_ID` produzidos
e grava um manifesto `*.iso.manifest.json` contendo o commit, a data do build
e o SHA-256 da imagem.
