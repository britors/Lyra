...
if [ ! -s "$GRUB_THEME_ASSET" ]; then
  echo "Missing installed-system GRUB theme: /$GRUB_THEME_ASSET" >&2
  exit 1
fi
...
# KIWI's live bootloader writer uses /boot/grub2/themes for the ISO menu and
# can overwrite the value previously written by the theme RPM/config.sh. The
# rootfs copied to the installed disk instead owns the theme below /usr/share.
