#!/usr/bin/env bash
# Validate and publish Lyra OS Desktop Alpha 3 artifacts to SourceForge FRS.
...
REMOTE_DIR="/home/frs/project/lyra/releases/1.0/desktop/alpha3/"
REMOTE="$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR"
PUBLIC_URL="https://sourceforge.net/projects/lyra/files/releases/1.0/desktop/alpha3/"
DOWNLOAD_URL="https://downloads.sourceforge.net/project/lyra/releases/1.0/desktop/alpha3"
...
usage() {
  cat <<EOF
Uso: ./scripts/upload-desktop-alpha3-sourceforge.sh [opções]
...
if [ "$VERIFY_DOWNLOAD" -eq 1 ] && ! command -v curl >/dev/null 2>&1; then
...
cleanup() {
  if [ -n "$DOWNLOAD_DIR" ]; then
    case "$DOWNLOAD_DIR" in
      /tmp/lyra-alpha3-download.*) rm -rf -- "$DOWNLOAD_DIR" ;;
...
echo "AVISO: Alpha 3 é publicada somente com SHA-256, sem assinatura GPG da ISO."
...
if manifest.get("version") != "2026.08-alpha3":
...
if [ "$VERIFY_DOWNLOAD" -eq 1 ]; then
  DOWNLOAD_DIR="$(mktemp -d /tmp/lyra-alpha3-download.XXXXXX)"
