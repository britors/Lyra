#!/usr/bin/env bash
# Validate and publish Lyra OS Desktop Alpha 5 artifacts to SourceForge FRS.
...
REMOTE_DIR="/home/frs/project/lyra/releases/1.0/desktop/alpha5/"
REMOTE="$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR"
PUBLIC_URL="https://sourceforge.net/projects/lyra/files/releases/1.0/desktop/alpha5/"
DOWNLOAD_URL="https://downloads.sourceforge.net/project/lyra/releases/1.0/desktop/alpha5"
...
usage() {
  cat <<EOF
Uso: ./scripts/upload-desktop-alpha5-sourceforge.sh [opções]
...
cleanup() {
  if [ -n "$DOWNLOAD_DIR" ]; then
    case "$DOWNLOAD_DIR" in
      /tmp/lyra-alpha5-download.*) rm -rf -- "$DOWNLOAD_DIR" ;;
...
echo "AVISO: Alpha 5 é publicada somente com SHA-256, sem assinatura GPG da ISO."
...
if manifest.get("version") != "2026.08-alpha5":
...
if [ "$VERIFY_DOWNLOAD" -eq 1 ]; then
  DOWNLOAD_DIR="$(mktemp -d /tmp/lyra-alpha5-download.XXXXXX)"
