#!/usr/bin/env bash
# Validate and publish Lyra OS Desktop Alpha 4 artifacts to SourceForge FRS.
...
REMOTE_DIR="/home/frs/project/lyra/releases/1.0/desktop/alpha4/"
REMOTE="$REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR"
PUBLIC_URL="https://sourceforge.net/projects/lyra/files/releases/1.0/desktop/alpha4/"
DOWNLOAD_URL="https://downloads.sourceforge.net/project/lyra/releases/1.0/desktop/alpha4"
...
usage() {
  cat <<EOF
Uso: ./scripts/upload-desktop-alpha4-sourceforge.sh [opções]
...
PREFIX="$(./scripts/release.py field iso_filename)"
PREFIX="${PREFIX%.iso}"
...
cleanup() {
  if [ -n "$DOWNLOAD_DIR" ]; then
    case "$DOWNLOAD_DIR" in
      /tmp/lyra-alpha4-download.*) rm -rf -- "$DOWNLOAD_DIR" ;;
...
echo "AVISO: Alpha 4 é publicada somente com SHA-256, sem assinatura GPG da ISO."
...
if manifest.get("version") != "2026.08-alpha4":
...
if [ "$VERIFY_DOWNLOAD" -eq 1 ]; then
  DOWNLOAD_DIR="$(mktemp -d /tmp/lyra-alpha4-download.XXXXXX)"
