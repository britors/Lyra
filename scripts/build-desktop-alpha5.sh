#!/usr/bin/env bash
# Build Lyra OS Desktop Alpha 5 and prepare the unsigned publication bundle.
...
EXPECTED_VERSION="2026.08-alpha5"
ARTIFACTS_ONLY=0

usage() {
  cat <<EOF
Uso: ./scripts/build-desktop-alpha5.sh [--artifacts-only]

Sem opções, constrói e valida uma nova ISO usando o instalador publicado no
OBS. --artifacts-only reutiliza a ISO Alpha 5 já validada e prepara o bundle
...
echo "Validando metadados do Lyra OS Desktop Alpha 5..."
...
echo "Bundle Alpha 5 pronto em: $ARTIFACT_DIR"
printf '  %s\n' \
  README.md \
  "$PREFIX.cdx.json" \
  "$PREFIX.iso" \
  ...
  "$PREFIX.spdx.json" \
  "$PREFIX.verified"
echo "Alpha 5 não usa assinatura GPG da ISO; ela passa a ser obrigatória na Beta 1."
