#!/usr/bin/env bash
# Build Lyra OS Desktop Alpha 4 and prepare the unsigned publication bundle.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
BUILD_DIR="$REPO_ROOT/kiwi/.kiwi/test-$(id -u)/build"
ARTIFACT_DIR="$REPO_ROOT/kiwi/.kiwi/test-$(id -u)/iso"
EXPECTED_VERSION="2026.08-alpha4"
ARTIFACTS_ONLY=0

usage() {
  cat <<EOF
Uso: ./scripts/build-desktop-alpha4.sh [--artifacts-only]

Sem opções, constrói e valida uma nova ISO usando o instalador publicado no
OBS. --artifacts-only reutiliza a ISO Alpha 4 já validada e prepara o bundle
de publicação sem executar novamente o KIWI.
EOF
}
...
echo "Validando metadados do Lyra OS Desktop Alpha 4..."
...
if [ "$ARTIFACTS_ONLY" -eq 0 ]; then
  ...
  sudo -v
  echo "Iniciando o build da ISO desktop com o instalador publicado no OBS..."
  echo "A VM existente não será alterada."
...
echo "Bundle Alpha 4 pronto em: $ARTIFACT_DIR"
printf '  %s\n' \
  README.md \
  "$PREFIX.cdx.json" \
  "$PREFIX.iso" \
  ...
  "$PREFIX.spdx.json" \
  "$PREFIX.verified"
echo "Alpha 4 não usa assinatura GPG da ISO; ela passa a ser obrigatória na Beta 1."
